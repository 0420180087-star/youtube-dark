...
export const OAuthCallback: React.FC = () => {
  ...
  useEffect(() => {
    const handle = async () => {
      ...
      try {
        setMessage('Buscando configurações do servidor...');

        let youtubeConfig = { client_id: '', client_secret: '' };
        
        try {
          const configRes = await fetch(`${supabaseUrl}/functions/v1/get-youtube-config`, {
            method: 'POST',
            headers: {
          ...
          console.warn('Could not fetch YouTube config from edge function, using fallback');
        }

        const clientId = youtubeConfig.client_id || import.meta.env.VITE_GOOGLE_CLIENT_ID;
        const clientSecret = youtubeConfig.client_secret;

        if (!clientId || !clientSecret) {
          throw new Error('Configurações do YouTube incompletas. Verifique as secrets no Supabase.');
        }
